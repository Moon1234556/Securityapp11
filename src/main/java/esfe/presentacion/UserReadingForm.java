...
import javax.swing.table.DefaultTableModel; // Importa la clase DefaultTableModel, utilizada para crear y manipular modelos de datos para JTables.
...
import java.util.ArrayList; // Importa la clase ArrayList, una implementación de la interfaz List que permite almacenar colecciones dinámicas.
...
public class UserReadingForm extends JDialog {
    ...
    public UserReadingForm(MainForm mainForm) {
        ...
        txtName .addKeyListener((KeyAdapter Released(e) -> {
        ...
        btnCreate.addActionListener(( s) -> {
            // Crea una nueva instancia de UserWriteForm para la creación de un nuevo usuario, pasando la MainForm, la constante CREATE de CUD y un nuevo objeto User.
        ...
        btnUpdate.addActionListener(( s) -> { // Note: 's' is often 'e' for event, but 's' is also valid.
            ...
            if (user != null) {
                // Crea una nueva instancia de UserWriteForm para la actualización del usuario seleccionado, pasando la MainForm, la constante UPDATE de CUD y el objeto User.
        ...
        btnDelete.addActionListener(( s) -> {
            ...
            // Verifica si se seleccionó un usuario en la tabla (getUserFromTableRow no devolvió null).
            if (user != null) {
                // Crea una nueva instancia de UserWriteForm para la eliminación del usuario seleccionado, pasando la MainForm, la constante DELETE de CUD y el objeto User.
    ...
    private User getUserFromTableRow() {
        ...
        try {
           ...
           // Verifica si se encontró un usuario con el ID proporcionado.
            if (user.getId() == 0) { // Ojo: Asegúrate de que 'user.getId() == 0' sea la forma correcta de verificar si no se encontró un usuario en tu implementación.
            ...
            //si se encuentra un usuario, lo retorna.
            return user;
        } catch (Exception ex) {
